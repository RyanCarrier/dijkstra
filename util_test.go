	...
	return Graph{
		...
		[]*Vertex{
			...
			&Vertex{sync.RWMutex{}, 2, 0, 0, map[int]int64{
				...
				4: 5},
				false, make(chan update, 1)},
			&Vertex{sync.RWMutex{}, 3, 0, 0, map[int]int64{}, false, make(chan update, 1)},
...
func getGGraph() (Graph, map[string]int) {
	return Graph{
			...
			[]*Vertex{
				&Vertex{sync.RWMutex{}, 0, 0, 0, map[int]int64{
					1: 2}, false, make(chan update, 1),
				},
				&Vertex{sync.RWMutex{}, 1, 0, 0, map[int]int64{
					2: 5}, false, make(chan update, 1),
				},
				&Vertex{sync.RWMutex{}, 2, 0, 0, map[int]int64{
					0: 1,
					1: 1}, false, make(chan update, 1),
				},
			},
			newLinkedList(),
			map[string]int{
	...
	return Graph{
		...
		[]*Vertex{
			...
			&Vertex{sync.RWMutex{}, 3, 0, 0, map[int]int64{
				2: 5},
				false, make(chan update, 1)},
			&Vertex{sync.RWMutex{}, 4, 0, 0, map[int]int64{}, false, make(chan update, 1)},
		},
		newLinkedList(),
		map[string]int{},
		false,
